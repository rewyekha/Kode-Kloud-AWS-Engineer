...
```bash
~ on   (us-east-1)   ls -l /root/.ssh/id_rsa
...
```bash
~ on   (us-east-1)  ssh-keygen -t rsa -b 2048 -f /root/.ssh/id_rsa -N ""
...
```bash
~ on   (us-east-1)   cat /root/.ssh/id_rsa.pub
...
```bash
~ on   (us-east-1)   ssh -i /root/.ssh/id_rsa root@<EC2_PUBLIC_IP>
